...
Although its target platform is the GPU, I've written it in a "bilingual" pattern of CUDA so that exactly the same code can be compiled as c++ to run on the CPU. A CPU will not be nearly as fast as a GPU, about an order of magnitude slower, in my experience. However, since one of the goals of this project was for the results we publish to be fully transparent and reproducible, I think it's important that as many people as possible have access to it.  I'm currently in the process of converting it into "trilingual", adding support for SYCL, which could allow operation on Intel and possibly AMD GPUs.
