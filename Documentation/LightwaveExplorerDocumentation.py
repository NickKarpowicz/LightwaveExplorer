...
def _(mo):
    ...
    14. x offset: amount by which to shift the beam on the spatial grid. Will always be 0 when radial symmetry is applied.
    15. z offset: amount by which to shift the beam relative to the propagation coordinate. 0 means that the beam is focused at the beginning of the simulation. If it is negative, it will be diverging as the simulation starts, and if it's positive, it will be converging.
    16. NC angle: Noncollinear angle of the beam. If it is 0, the beam will be pointing along the z-axis.
    17. Polarization: Angle in degrees of the field direction relative to the x-y plane. 0 means the field is along the x axis, 90 means it is along the y axis.
    18. Circularity: degree of ellipticity of the field. 0 means linearly polarized, 1 means circular.
...
def _(mo):
    ...
    #### offsetAperture(diameter, activationParameter, xOffset, yOffset)
    This is similar to aperture, with two extra parameters to offset it from the center of the grid (yOffset only applies in 3D modes, and this function cannot be used under radial symmetry assumptions).
