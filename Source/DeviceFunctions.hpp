...
deviceFunction static void
expandCylindricalBeamDevice(const deviceParameterSet<T, U> *s, const int64_t i,
                            T *expandedBeam1, const T *sourceBeam1,
                            const T *sourceBeam2) {
...
deviceFunction static complex_t
fourierPropagator(const complex_t k, const real_t dk1, const real_t dk2,
                  const real_t k0, const real_t d) {

  if (isnan(k0) || isnan(dk1) || isnan(dk2) || isComplexNaN(k) ||
      k.real() == real_t{})
    return complex_t{};
  if ((deviceFPLib::abs(dk2) < 0.1f * k.real()) &&
    ...
    complex_t returnVal = complex_t(d * k.imag(), d * (halfOverKr - kMoving));
    return isComplexNaN(returnVal) ? complex_t{} : deviceLib::exp(returnVal);
  }
  complex_t kz = (deviceLib::sqrt(-dk2 * dk2 / (k + deviceFPLib::abs(dk1)) + k -
                                  deviceFPLib::abs(dk1)) *
                      deviceLib::sqrt(k + deviceFPLib::abs(dk1)) -
                  k0);
  if (kz.imag() > 0.0f)
    kz = complex_t(kz.real(), -kz.imag());
...
// macro for the two versions because I don't want to lose precision in the
// double version but Intel Xe graphics need the constants as floats, and CUDA
// doesn't work with the
//[[maybe_unused]] attribute... and I don't like compiler warnings.
[[maybe_unused]] deviceFunction static float deviceDawson(const float x) {

  // parameters determining accuracy (higher n, smaller h -> more accurate but
  // slower)
  const int n = 15;
  const float h = 0.3f;
  ...
  if (deviceFPLib::abs(x) < 0.2f) {
    ...
    return x * (1.0f - 2.0f * x2 / 3.0f + 4.0f * x4 / 15.0f -
                8.0f * x2 * x4 / 105.0f + (16.0f / 945.0f) * x4 * x4 -
...
[[maybe_unused]] deviceFunction static double deviceDawson(const double x) {
  ...
  if (deviceFPLib::abs(x) < 0.2) {
    ...
    return x * (1.0 - 2.0 * x2 / 3.0 + 4.0 * x4 / 15.0 - 8.0 * x2 * x4 / 105.0 +
                (16.0 / 945) * x4 * x4 - (32.0 / 10395) * x4 * x4 * x2);
  }
...
deviceFunction static deviceComplex
sellmeierFunc(deviceFP ls, const deviceFP omega, const deviceFP *a,
              const int eqn, const bool takeSqrt = true) {
  ...
  case 0:
    [[unlikely]]
    if (ls == -a[3] || ls == -a[6] || ls == -a[9] || ls == -a[12])
      return deviceComplex{};
    realPart = a[0] + (a[1] + a[2] * ls) / (ls + a[3]) +
               (a[4] + a[5] * ls) / (ls + a[6]) +
               (a[7] + a[8] * ls) / (ls + a[9]) +
               (a[10] + a[11] * ls) / (ls + a[12]) + a[13] * ls +
               a[14] * ls * ls + a[15] * ls * ls * ls;
    compPart = kLorentzian<deviceFP>() * a[16] /
                   deviceComplex(a[17] - omega2, a[18] * omega) +
               kLorentzian<deviceFP>() * a[19] /
                   deviceComplex(a[20] - omega2, a[21] * omega);
  ...
  case 2: {
    ...
    for (int i = 0; i < 7; ++i) {
      if (a[3 * i + 1] != 0.0f) {
        ...
        compPart +=
            a[3 + 3 * i] *
            deviceComplex(-invSqrtPi<deviceFP>() * deviceDawson(scaledF),
  ...
  case 100: {
    [[unlikely]] if (ls == -a[3] || ls == -a[6] || ls == -a[9] || ls == -a[12])
      return deviceComplex{};
  }
    realPart = a[0] + (a[1] + a[2] * ls) / (ls + a[3]) +
               (a[4] + a[5] * ls) / (ls + a[6]) +
    ...
    return takeSqrt
               ? deviceComplex(deviceFPLib::sqrt(maxN(realPart, 0.9f)), 0.0f)
               : maxN(realPart, 0.9f);
  default:
...
deviceFunction static inline deviceFP square(const deviceFP x) { return x * x; }

// Sellmeier equation for refractive indices
template <typename deviceFP, typename deviceComplex>
deviceFunction static deviceFP
sellmeierDevice(deviceComplex *ne, deviceComplex *no, const deviceFP *a,
                const deviceFP f, const deviceFP theta, const deviceFP phi,
                const int type, const int eqn, const bool takeSqrt = true) {
  ...
  // option 1: uniaxial
    ...
    deviceComplex na =
        sellmeierFunc<deviceFP, deviceComplex>(ls, omega, a, eqn, false);
    deviceComplex nb =
        sellmeierFunc<deviceFP, deviceComplex>(ls, omega, &a[22], eqn, false);
  ...
  // option 2: biaxial
    ...
    deviceComplex nb =
        sellmeierFunc<deviceFP, deviceComplex>(ls, omega, &a[22], eqn, false);
    deviceComplex nc =
        sellmeierFunc<deviceFP, deviceComplex>(ls, omega, &a[44], eqn, false);
    ...
    deviceFP d = (na.real() == nb.real())
                     ? deviceFP{}
                     : 0.5f * deviceFPLib::atan(
                                  deviceFPLib::sin(2 * phi) * ct /
                                  (((1.0f / nb.real() - 1.0f / nc.real()) /
                                    (1.0f / na.real() - 1.0f / nb.real())) *
                                       st * st -
                                   cp * cp * ct * ct + sp * sp));
...
deviceFunction static inline deviceFP modulusSquared(const deviceComplex &a) {
...
deviceFunction static deviceFP
rhoInRadialSymmetry(const int64_t N, const int64_t j, const deviceFP dr) {
...
// function to find the effective crystal direction (theta,phi) for a given
...
//   Using conservation of the transverse momentum, we must solve the system of
//   equations:
//	kx1 = kx2
//   ky1 = ky2
//   where
...
//
//  The k grid is known, meaning kx1 and ky1 are givens; alpha and beta are
//  unknowns minimize n(alpha,beta)*sin(alpha) - kx1*c/w,
//  n(alpha,beta)*sin(beta) - ky1*c/w
//
//  starting point n = n(alpha=0, beta=0), which is the actual solution for
//  isotropic medium If isotropic, return If uniaxial, solve 1D problem with
//  n(alpha,0) If biaxial, solve 2D problem Use OGM1; D. Kim, J.A. Fessler,
//  Optimized first-order methods for smooth convex minimization,
//  arXiv:1406.5468
template <typename deviceFP, typename deviceComplex>
deviceFunction static deviceFP findBirefringentCrystalIndex(
    const deviceParameterSet<deviceFP, deviceComplex> *s,
    const deviceFP *sellmeierCoefficients, const int64_t i, deviceComplex *n1,
    deviceComplex *n2) {
  ...
  deviceFP kx1 =
      (lightC<deviceFP>() / (twoPi<deviceFP>() * f)) *
      (j * (*s).dk1 - (j >= ((*s).Nspace / 2)) * ((*s).dk1 * (*s).Nspace));
  deviceFP ky1 = (*s).is3D ? (lightC<deviceFP>() / (twoPi<deviceFP>() * f)) *
                                 (k * (*s).dk2 - (k >= ((*s).Nspace2 / 2)) *
                                                     ((*s).dk2 * (*s).Nspace2))
                           : deviceFP{};
  ...
  sellmeierDevice(&n[0][0], &n[0][1], sellmeierCoefficients, f,
                  (*s).crystalTheta, (*s).crystalPhi, (*s).axesNumber,
                  (*s).sellmeierType);
  if ((*s).axesNumber == 0) {
    *n1 = n[0][0];
    *n2 = n[0][1];
  ...
  deviceFP alpha[2] = {deviceFPLib::asin(kx1 / n[0][0].real()),
                       deviceFPLib::asin(kx1 / n[0][1].real())};
  deviceFP beta[2] = {deviceFPLib::asin(ky1 / n[0][0].real()),
                      deviceFPLib::asin(ky1 / n[0][1].real())};
  ...
  if ((*s).axesNumber == 1) {
    maxiter = 64;
    sellmeierDevice(&n[0][0], &nW, sellmeierCoefficients, f,
                    (*s).crystalTheta + alpha[0] + gradientStep,
                    (*s).crystalPhi, (*s).axesNumber, (*s).sellmeierType);
    sellmeierDevice(&n[1][0], &nW, sellmeierCoefficients, f,
                    (*s).crystalTheta + alpha[0] - gradientStep,
                    (*s).crystalPhi, (*s).axesNumber, (*s).sellmeierType);
    if (isComplexNaN(n[0][0]) || isComplexNaN(n[1][0])) {
      *n1 = deviceComplex{};
      *n2 = deviceComplex{};
      return deviceFP{};
    }
    errArray[0][0] =
        deviceFPLib::sin(alpha[0] + gradientStep) * n[0][0].real() - kx1;
    errArray[1][0] =
        deviceFPLib::sin(alpha[0] - gradientStep) * n[1][0].real() - kx1;
    ...
    sellmeierDevice(&n[0][0], &nW, sellmeierCoefficients, f,
                    (*s).crystalTheta + alpha[0], (*s).crystalPhi,
                    (*s).axesNumber, (*s).sellmeierType);
    sellmeierDevice(&nW, &n[1][1], sellmeierCoefficients, f,
                    (*s).crystalTheta + alpha[1], (*s).crystalPhi,
                    (*s).axesNumber, (*s).sellmeierType);
    *n1 = n[0][0];
    *n2 = n[1][1];
    return deviceFP{};
  ...
  if ((*s).axesNumber == 2) {
    sellmeierDevice(&n[0][0], &nW, sellmeierCoefficients, f,
                    (*s).crystalTheta + alpha[0] + gradientStep,
                    (*s).crystalPhi + beta[0], (*s).axesNumber,
                    (*s).sellmeierType);
    sellmeierDevice(&n[1][0], &nW, sellmeierCoefficients, f,
                    (*s).crystalTheta + alpha[0] - gradientStep,
                    (*s).crystalPhi + beta[0], (*s).axesNumber,
                    (*s).sellmeierType);
    sellmeierDevice(&n[2][0], &nW, sellmeierCoefficients, f,
                    (*s).crystalTheta + alpha[0],
                    (*s).crystalPhi + beta[0] + gradientStep, (*s).axesNumber,
                    (*s).sellmeierType);
    sellmeierDevice(&n[3][0], &nW, sellmeierCoefficients, f,
                    (*s).crystalTheta + alpha[0],
                    (*s).crystalPhi + beta[0] - gradientStep, (*s).axesNumber,
                    (*s).sellmeierType);
    sellmeierDevice(&nW, &n[0][1], sellmeierCoefficients, f,
                    (*s).crystalTheta + alpha[1] + gradientStep,
                    (*s).crystalPhi + beta[1], (*s).axesNumber,
                    (*s).sellmeierType);
    sellmeierDevice(&nW, &n[1][1], sellmeierCoefficients, f,
                    (*s).crystalTheta + alpha[1] - gradientStep,
                    (*s).crystalPhi + beta[1], (*s).axesNumber,
                    (*s).sellmeierType);
    sellmeierDevice(&nW, &n[2][1], sellmeierCoefficients, f,
                    (*s).crystalTheta + alpha[1],
                    (*s).crystalPhi + beta[1] + gradientStep, (*s).axesNumber,
                    (*s).sellmeierType);
    sellmeierDevice(&nW, &n[3][1], sellmeierCoefficients, f,
                    (*s).crystalTheta + alpha[1],
                    (*s).crystalPhi + beta[1] - gradientStep, (*s).axesNumber,
                    (*s).sellmeierType);
    if (isComplexNaN(n[0][0]) || isComplexNaN(n[1][0])) {
    ...
    errArray[1][0] =
        deviceFPLib::sin(alpha[0] - gradientStep) * n[1][0].real() - kx1;
    errArray[2][0] =
        deviceFPLib::sin(beta[0] + gradientStep) * n[2][0].real() - ky1;
    errArray[3][0] =
        deviceFPLib::sin(beta[0] - gradientStep) * n[3][0].real() - ky1;
    errArray[0][1] =
        deviceFPLib::sin(alpha[1] + gradientStep) * n[0][1].real() - kx1;
    errArray[1][1] =
        deviceFPLib::sin(alpha[1] - gradientStep) * n[1][1].real() - kx1;
    errArray[2][1] =
        deviceFPLib::sin(beta[1] + gradientStep) * n[2][1].real() - ky1;
    errArray[3][1] =
        deviceFPLib::sin(beta[1] - gradientStep) * n[3][1].real() - ky1;
    gradient[0][0] = gradientFactor * (errArray[0][0] - errArray[1][0]);
    ...
    for (it = 0; it < maxiter; ++it) {
      ...
      if (deviceFPLib::abs(gradient[0][0]) > 1e-2f)
        alpha[0] -= 0.25f * (errArray[0][0] + errArray[1][0]) / gradient[0][0];
      if (deviceFPLib::abs(gradient[1][0]) > 1e-2f)
        beta[0] -= 0.25f * (errArray[2][0] + errArray[3][0]) / gradient[1][0];
      if (deviceFPLib::abs(gradient[0][1]) > 1e-2f)
        alpha[1] -= 0.25f * (errArray[0][1] + errArray[1][1]) / gradient[0][1];
      if (deviceFPLib::abs(gradient[1][1]) > 1e-2f)
        beta[1] -= 0.25f * (errArray[2][1] + errArray[3][1]) / gradient[1][1];

      if (maxN(maxN(deviceFPLib::abs(gradient[0][0]),
                    deviceFPLib::abs(gradient[1][0])),
               maxN(deviceFPLib::abs(gradient[0][1]),
                    deviceFPLib::abs(gradient[1][1]))) < gradientTol)
        break;
      sellmeierDevice(&n[0][0], &nW, sellmeierCoefficients, f,
                      (*s).crystalTheta + alpha[0] + gradientStep,
                      (*s).crystalPhi + beta[0], (*s).axesNumber,
    ...
    deviceFP d = sellmeierDevice(
        &n[0][0], &nW, sellmeierCoefficients, f, (*s).crystalTheta + alpha[0],
        (*s).crystalPhi + beta[0], (*s).axesNumber, (*s).sellmeierType);
    sellmeierDevice(&nW, &n[1][1], sellmeierCoefficients, f,
                    (*s).crystalTheta + alpha[1], (*s).crystalPhi + beta[1],
                    (*s).axesNumber, (*s).sellmeierType);
    *n1 = n[0][0];
    *n2 = n[1][1];
    return d;
...
template <typename deviceFP> class SimpsonIntegrator {
...
public:
  ...
  deviceFunction inline void step(const deviceFP newIntegrandValue) {
    ...
    integralValues[2] =
        integralValues[0] +
        third<deviceFP>() * (integrandValues[0] + 4.0f * integrandValues[1] +
                             integrandValues[2]);
...
template <typename deviceFP> class TrapezoidIntegrator {
  deviceFP integrandValues[2]{};
  deviceFP integralValues[2]{};

public:
  deviceFunction TrapezoidIntegrator() {}
  deviceFunction TrapezoidIntegrator(const deviceFP initialValue) {
    integralValues[0] = initialValue;
    integralValues[1] = initialValue;
  }
  deviceFunction inline void step(const deviceFP newIntegrandValue) {
    integrandValues[0] = integrandValues[1];
    integrandValues[1] = newIntegrandValue;
    integralValues[1] =
        integralValues[0] + 0.5f * (integrandValues[0] + integrandValues[1]);
    integralValues[0] = integralValues[1];
  }

  deviceFunction inline deviceFP current_integral() {
    return integralValues[1];
  }
};
} // namespace deviceFunctions
