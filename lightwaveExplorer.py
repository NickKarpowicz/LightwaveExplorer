...
def load(filePath: str, loadFieldArray=True):
    ...
    s.sgOrder2 = readLine(lines[7])
    s.cephase1 = readLine(lines[8])
    s.cephase2 = readLine(lines[9])
    s.delay1 = readLine(lines[10])
    s.delay2 = readLine(lines[11])
    s.gdd1 = readLine(lines[12])
    s.gdd2 = readLine(lines[13])
    s.tod1 = readLine(lines[14])
    s.tod2 = readLine(lines[15])
    s.phaseMaterialIndex1 = readLine(lines[16])
    s.phaseMaterialIndex2 = readLine(lines[17])
    s.phaseMaterialThickness1 = readLine(lines[18])
    s.phaseMaterialThickness2 = readLine(lines[19])
    s.beamwaist1 = readLine(lines[20])
    s.beamwaist2 = readLine(lines[21])
    s.x01 = readLine(lines[22])
    s.x02 = readLine(lines[23])
    s.z01 = readLine(lines[24])
    s.z02 = readLine(lines[25])
    s.propagationAngle1 = readLine(lines[26])
    s.propagationAngle2 = readLine(lines[27])
    s.polarizationAngle1 = readLine(lines[28])
    s.polarizationAngle2 = readLine(lines[29])
    s.circularity1 = readLine(lines[30])
    s.circularity2 = readLine(lines[31])
    s.materialIndex = readLine(lines[32])
    s.materialIndexAlternate = readLine(lines[33])
    s.crystalTheta = readLine(lines[34])
    s.crystalPhi = readLine(lines[35])
    s.spatialWidth = readLine(lines[36])
    s.spatialHeight = readLine(lines[37])
    s.rStep = readLine(lines[38])
    s.timeSpan = readLine(lines[39])
    s.tStep = readLine(lines[40])
    s.crystalThickness = readLine(lines[41])
    s.propagationStep = readLine(lines[42])
    s.nonlinearAbsorptionStrength = readLine(lines[43])
    s.bandGapElectronVolts = readLine(lines[44])
    s.effectiveMass = readLine(lines[45])
    s.drudeGamma = readLine(lines[46])
    s.symmetryType = readLine(lines[47])
    s.batchIndex = readLine(lines[48])
    s.batchDestination = readLine(lines[49])
    s.Nsims = int(readLine(lines[50]))
    testString = lines[51]
    if testString[0] == 'B':
        s.batchIndex2 = readLine(lines[51])
